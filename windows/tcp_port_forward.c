...
static void free_session(struct session *session, sessions *head, HANDLE epfd) {
  if (session == NULL) {
    return;
  }
...
/// set.
static bool set_non_blocking(int sockfd) { return true; }
  ...
  for (;;) {
    ...
    for (int i = 0; i < nfds; i++) {
      ...
      if (events[i].events & EPOLLOUT) {
        ...
        if (getsockopt(events[i].data.fd, SOL_SOCKET, SO_ERROR, buf,
                       &err_len) == -1) {
          close(session->client_fd);
          ...
          printf(
              "[+] port forwarding tunnel is opened now,client socket "
      ...
      if (events[i].events & EPOLLIN) {
        ...
        if (session != NULL) {
          ...
          read_size = recv(events[i].data.fd, buf, BUF_SIZE, 0);
          if (read_size == -1) {
          ...
          send(session->remote_fd, buf, read_size, 0);
          continue;
        }
        session = query_remote_session(&head, events[i].data.fd);
        read_size = recv(events[i].data.fd, buf, BUF_SIZE, 0);
        send(session->client_fd, buf, read_size, 0);
        continue;
      }
    }
  }
